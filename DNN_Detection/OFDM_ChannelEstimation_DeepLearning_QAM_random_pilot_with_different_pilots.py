...
def ofdm_simulate(codeword, channelResponse,SNRdb):   
    ...
    #QAM = 2*bits - 1 + (2*np.random.binomial(n=1, p=0.5, size=(payloadBits_per_OFDM, ))-1)*1j
    ...
    #OFDM_demod[dataCarriers] = 0
    #OFDM_RX_noCP = IDFT(OFDM_demod)
    #pilots = DFT(OFDM_RX_noCP)[pilotCarriers])

    # ----- target inputs ---
    symbol = np.zeros(K, dtype=complex)
    codeword_qam = Modulation(codeword)
    #codeword_qam = 2*codeword - 1 +(2*np.random.binomial(n=1, p=0.5, size=(K, ))-1)*1j
    #print ('before assignment', symbol)
    #symbol[np.arange(K)] = codeword_qam
    if len(codeword_qam) != K:
        print ('length of code word is not equal to K, error !!')
    symbol = codeword_qam
    #print ('after assignment', symbol)
    #print ('code word qam', codeword_qam)
    ...
    #print(symbol.size,OFDM_time_codeword.size, OFDM_RX_codeword.size, OFDM_RX_noCP.size,OFDM_RX_noCP_codeword.size )
...
def training():        
        ...
        info_len = 32
        code_len = 64
        #bits = np.random.binomial(n=1, p=0.5, size=(36, ))
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(traing_epochs):
                ...
                if epoch % display_step == 0:
                    ...
                    print("OFDM Detection QAM output number is 4, SNR = ", SNRdb, "Num Pilot", P,"prediction and the mean error on test set are:", mean_error.eval({X:batch_x}), mean_error_rate.eval({X:batch_x}))
